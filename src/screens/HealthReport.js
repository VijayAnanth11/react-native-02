...
const HealthReport = ({ navigation }) => {
    ...
    const Sendtobackend = () => {
        if (fdata.date== '' ||
            ...
            fetch('http://192.168.159.210:3000/health', {
                method: 'POST',
                headers: {
                ...
                .then(res => res.json()).then(
                    data => {
                        ...
                        if (data.error) {
                            ...
                            alert('Health Details added successfully');
    ...
    return (
        <ImageBackground source={p13} style={styles.container}>
            ...
            <View style={styles.root}>
                <Text style={styles.sub}>Add Health Details</Text>
                {
                ...
                <View style={styles.name2}>
                    ...
                    <TextInput style={styles.userinput3} placeholder="Enter name"
                        onPressIn={() => setErrormsg(null)}
                        onChangeText={(text) => setFdata({ ...fdata, name: text })}
                    />
                </View>
                <View style={styles.name2}>
                    <Text style={styles.nametext}>Health Condition</Text>
                    <TextInput style={styles.userinput3} placeholder="Health Details"
                        onPressIn={() => setErrormsg(null)}
                        onChangeText={(text) => setFdata({ ...fdata, condition: text })}
                    />
                </View>
